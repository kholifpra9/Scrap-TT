...
function checkRateLimit(req, res) {
  ...
  if (info.count >= RATE_LIMIT_MAX) {
    ...
    const resetStr = resetDate.toLocaleTimeString("id-ID", {
      hour: "2-digit",
      minute: "2-digit",
    });
    res.status(429).json({
...
function cleanOldFolders() {
  ...
  const foldersWithTimestamp = scrapeFolders
    .map((folder) => {
  ...
  const toDelete = foldersWithTimestamp
    .slice(0, Math.max(0, foldersWithTimestamp.length - KEEP_FOLDERS))
    .map((f) => f.name);
...
app.post("/api/scrape-comments", (req, res) => {
  ...
  const proc = spawn(
    `npx tsx parse-comments.ts ${encodedQuery} ${videoCount} ${commentCount}`,
    {
      cwd: __dirname,
      env: {
        ...process.env,
  ...
  proc.on("close", (code) => {
    ...
    const allEntries = fs.existsSync(RESULT_DIR)
      ? fs.readdirSync(RESULT_DIR)
      : [];
    ...
    if (scrapeFolder) {
      ...
      if (!job.commentsCsv)
        job.commentsCsv = `result/${scrapeFolder}/comments.csv`;
    }
    broadcast(clientId, {
      type: "done",
      jobId,
      status: job.status,
      videosCsv: job.videosCsv,
      commentsCsv: job.commentsCsv,
      videoCount: job.videoCountResult,
      commentCount: job.commentCountResult,
      duration: ((job.endTime - job.startTime) / 1000).toFixed(1),
    });
    cleanOldFolders();
  });
...
app.post("/api/analyze", async (req, res) => {
  ...
  if (!apiKey) {
    return res
      .status(500)
      .json({
        error: "GROQ_API_KEY belum diset di server. Tambahkan ke file .env",
      });
  }
  if (!commentsCsv)
    return res.status(400).json({ error: "commentsCsv required" });
  ...
  if (!comments.length)
    return res.status(400).json({ error: "No comments found in file" });

  if (!context?.trim()) {
    return res
      .status(400)
      .json({
        error: "Prompt tidak boleh kosong. Tulis instruksi analisis kamu dulu.",
      });
  }
...
---`;
  ...
  try {
    ...
    if (!result)
      return res.status(500).json({ error: "Groq tidak mengembalikan hasil" });
...
server.listen(PORT, () => {
  ...
  if (!process.env.GROQ_API_KEY) {
    console.warn(
      "⚠️  GROQ_API_KEY belum diset. Tambahkan ke file .env untuk fitur AI Analysis.",
    );
  } else {
